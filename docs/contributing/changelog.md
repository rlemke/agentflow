...
**Run safety**: handler **preflight** — `run` refuses with `CatalogRunBlocked` if any event facet reachable from the entry workflow has no loadable handler in the fleet registry (catches declared-but-unimplemented facets before launch, instead of dead-lettering mid-flight). `RegistryDispatcher.check_loadable` imports each handler **and AST-scans its imports — including lazy `from x import y` inside the handler body** (skipping `try/except` and `TYPE_CHECKING` guards) — so a broken lazy import is caught at preflight too. **Execution isolation** — each run mints a fresh execution `workflow_id` + per-run `WorkflowDefinition` (was reusing the revision's definition id, so a prior terminated run's state collided with the next).
