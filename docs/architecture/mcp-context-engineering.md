...
The validator emits **41 distinct rule_ids** across these categories:

- **Placement** — `WORKFLOW_AT_TOP_LEVEL`, `SCHEMA_AT_TOP_LEVEL`
- **Reference resolution** — `REF_UNKNOWN_FACET`, `REF_AMBIGUOUS_FACET`, `REF_UNKNOWN_SCHEMA`, `REF_AMBIGUOUS_SCHEMA`, `REF_UNKNOWN_TYPE`, `REF_UNDEFINED_STEP`, `REF_INVALID_STEP_FORMAT`, `REF_INVALID_STEP_ATTRIBUTE`, `REF_INVALID_INPUT`, `REF_CROSS_BLOCK_STEP`, `REF_FORWARD_STEP`, `STEP_REF_FACET_MISMATCH`
...
42 files (41 rules + `PARSE_ERROR`), one per emitted rule_id, all following the same template:
