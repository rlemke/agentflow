...
Second, the suggested fixes and the docs uri are valuable to a *human reviewer* of agent-generated FFL, not only to the agent. A reviewer reading a generated workflow can click through to `afl://docs/rules/REF_INVALID_STEP_FORMAT` and see the same wrong/right framing the agent saw. The diagnostic surface is, in this sense, another instance of the projection layer argued for in §2.2: the same artifact serves machine repair and human verification. A pure-IR system would have to invent a separate review affordance for this; FFL inherits it because the surface and the semantics are paired.

This does not, on its own, prove the larger positioning. What it does demonstrate is that one of the load-bearing claims — *restricted semantics make AI-driven repair tractable in the same way restricted semantics make human review tractable* — is not aspirational. It is now the working contract between the compiler and any agent (Claude Code, an MCP client, a future planning agent) that touches FFL. The next claims to test are at the runtime layer: structured repair diagnostics for stuck workflows (the analogue at the execution layer of what the validator now does at the source layer), and effect annotations on event facets (so policy checks have something concrete to check against). Those follow the same template — a small, restricted vocabulary, paired with structured diagnostics and per-term docs — and they extend the same evidence.
