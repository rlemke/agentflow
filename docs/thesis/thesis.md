...
A stress test against the running system confirms the safety property under contention. Ten `osm-geocoder` replicas all polling the same task list, 100 concurrent workflows submitted within a 2-second window, produced 2 918 distinct step-level task documents. Of those, exactly zero `step_id` values reached a `completed` state through more than one task document — the partial unique index on `(step_id, state="running")` is the only mechanism enforcing the constraint, and it held across the entire run. This is the safety claim's empirical content: not "no observed duplicates by accident," but "no path through the protocol that could produce one." The skewed claim distribution under the same test (the fastest replica claimed 5× the slowest, see §5.4.1) is independent of safety — fairness is what suffers when N replicas race, not correctness.
