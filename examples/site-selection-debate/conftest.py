"""Root conftest for site-selection-debate -- cross-example module isolation."""
...
def _ensure_ssd_handlers():
    for key in list(sys.modules.keys()):
        if key == "handlers" or key.startswith("handlers."):
            mod = sys.modules[key]
            mod_file = getattr(mod, "__file__", "") or ""
            if "site-selection-debate" not in mod_file:
                del sys.modules[key]
