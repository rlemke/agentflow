"""Root conftest for event-driven-etl example.
...
def _ensure_etl_handlers():
    """Purge non-etl handler modules from sys.modules."""
    for key in list(sys.modules.keys()):
        if key == "handlers" or key.startswith("handlers."):
            mod = sys.modules[key]
            mod_file = getattr(mod, "__file__", "") or ""
            if "event-driven-etl" not in mod_file:
                del sys.modules[key]
