"""Root conftest for multi-round-debate -- cross-example module isolation."""
...
def _ensure_rounds_handlers():
    for key in list(sys.modules.keys()):
        if key == "handlers" or key.startswith("handlers."):
            mod = sys.modules[key]
            mod_file = getattr(mod, "__file__", "") or ""
            if "multi-round-debate" not in mod_file:
                del sys.modules[key]
