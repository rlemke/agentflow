"""Test-level conftest for devops-deploy example."""
...
def _ensure_deploy_handlers():
    """Purge non-deploy handler modules from sys.modules."""
    for key in list(sys.modules.keys()):
        if key == "handlers" or key.startswith("handlers."):
            mod = sys.modules[key]
            mod_file = getattr(mod, "__file__", "") or ""
            if "devops-deploy" not in mod_file:
                del sys.modules[key]
...
_ensure_deploy_handlers()


@pytest.fixture(autouse=True)
def _deploy_handlers_on_path():
